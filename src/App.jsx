    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div onClick={e => e.stopPropagation()} className={`relative z-10 rounded-3xl max-w-xl w-full max-h-[88vh] overflow-y-auto shadow-2xl ${dark ? "bg-stone-900 text-stone-100" : "bg-amber-50 text-stone-800"}`}>
        <div className="relative h-52 overflow-hidden rounded-t-3xl">
          ...
          {(item.protein || item.cal) && (
            <div className="absolute bottom-3 right-3 flex gap-2">
              {item.protein && <span className="bg-green-500/90 backdrop-blur-sm text-white text-xs font-bold px-2.5 py-1 rounded-lg shadow-md">{item.protein} Protein</span>}
              {item.cal && <span className="bg-green-500/90 backdrop-blur-sm text-white text-xs font-bold px-2.5 py-1 rounded-lg shadow-md">{item.cal} Cal</span>}
            </div>
          )}
        ...
        <div className="p-5">
          ...
          <ul className="grid grid-cols-2 gap-1.5 mb-5">
            {(lang === "si" && item.ingredientsSi ? item.ingredientsSi : item.ingredients).map((i, idx) => (
          ...
          <ol className="space-y-2">
            {(lang === "si" && item.stepsSi ? item.stepsSi : item.steps).map((s, i) => (
          ...
          {item.videoLink && (
            <div className={`mt-6 p-4 rounded-xl border ${dark ? "bg-stone-800/80 border-stone-700" : "bg-amber-100/50 border-amber-200"}`}>
              <h4 className={`font-bold mb-2 ${fs.card} ${dark ? "text-amber-400" : "text-amber-700"}`}>
                💡 {lang === "si" ? "ප්‍රධාන කෝකියාගේ ඉඟිය" : "Chef's Tip"}
              </h4>
              <a href={item.videoLink} target="_blank" rel="noopener noreferrer" className={`inline-flex items-center gap-2 font-semibold transition-colors hover:underline ${fs.base} ${dark ? "text-stone-300 hover:text-amber-400" : "text-stone-600 hover:text-amber-600"}`}>
                👉 {lang === "si" ? "වීඩියෝ නිබන්ධනය මෙතනින් නරඹන්න" : "Watch the video tutorial here"}
              </a>
            </div>
          )}
        </div>
    ...
    <div className="space-y-16">
      ...
      <section className={`py-12 ${dark ? "bg-stone-900" : "bg-amber-100"}`}>
        <div className="max-w-4xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[["🍄", "4+", "Mushroom Types"], ["📖", "20+", "Recipes"], ["🏪", "10+", "Partner Shops"], ["🌿", "100%", "Organic Options"]].map(([ic, v, l]) => (
            <div key={l}>
...
function RecipesPage({ dark, fs, lang }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {/* How to Clean Mushrooms Section */}
      <div className={`mb-12 rounded-3xl p-6 md:p-8 shadow-lg ${dark ? "bg-stone-800/80 border border-stone-700" : "bg-amber-100/50 border border-amber-200"}`}>
        <div className="flex items-center gap-3 mb-6">
          <span className="text-3xl">🧼</span>
          <h3 className={`font-extrabold font-serif ${fs.heading} ${dark ? "text-stone-100" : "text-stone-800"}`}>
            {t.cleanMushrooms.title}
          </h3>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
          {t.cleanMushrooms.steps.map((step, idx) => (
            <div key={idx} className={`p-4 rounded-2xl shadow-sm ${dark ? "bg-stone-900/50" : "bg-white/80"}`}>
              <h4 className={`font-bold mb-2 ${fs.card} ${dark ? "text-amber-400" : "text-amber-600"}`}>
                {step.title}
              </h4>
              <p className={`leading-relaxed ${fs.sub} ${dark ? "text-stone-400" : "text-stone-600"}`}>
                {step.desc}
              </p>
            </div>
          ))}
        ...
        {/* Pro Tip */}
        <div className={`p-5 rounded-2xl border ${dark ? "bg-amber-900/20 border-amber-700/50" : "bg-amber-50 border-amber-300"}`}>
          <h4 className={`font-bold mb-1 flex items-center gap-2 ${fs.card} ${dark ? "text-amber-400" : "text-amber-700"}`}>
            {t.cleanMushrooms.proTipTitle} <span className="italic opacity-90">{t.cleanMushrooms.proTipSub}</span>
          </h4>
          <p className={`${fs.base} ${dark ? "text-stone-300" : "text-stone-700"} leading-relaxed`}>
            {t.cleanMushrooms.proTipDesc}
          </p>
        </div>
      </div>
      ...
      <p className={`text-center mb-8 ${fs.base} ${dark ? "text-stone-400" : "text-stone-500"}`}>Discover our collection of mushroom-forward dishes.</p>
    ...
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
        {[
          ...
          { icon: <Icon.Phone />, label: t.phone, val: "0712518433", color: "text-green-400", link: "tel:0712518433" },
          { icon: <Icon.Mail />, label: t.email, val: "shenal@gmail.com", color: "text-amber-400", link: "mailto:shenal@gmail.com" },
        ].map(({ icon, label, val, color, link }) => (
          <div key={label} className={`rounded-2xl p-6 text-center shadow-md ${dark ? "bg-stone-800" : "bg-white"}`}>
            ...
            {link ? (
              <a href={link} className={`${fs.sub} hover:underline ${dark ? "text-amber-400" : "text-amber-600"}`}>{val}</a>
            ) : (
      ...
      {/* Map placeholder */}
      <div className={`relative rounded-2xl overflow-hidden shadow-md h-72 mb-8 ${dark ? "bg-stone-800" : "bg-amber-100"}`}>
        <iframe
          src="https://www.google.com/maps/embed?pb=!1m17!1m12!1m3!1d3959.532392944443!2d79.97608807423548!3d7.064096316646804!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m2!1m1!2s!5e0!3m2!1sen!2slk!4v1777185910906!5m2!1sen!2slk"
        ...
        ></iframe>
        <a
          href="https://www.google.com/maps/search/?api=1&query=7.064096,79.976088"
          target="_blank"
          rel="noopener noreferrer"
          className={`absolute bottom-4 right-4 px-4 py-2 rounded-xl font-bold shadow-lg transition-transform hover:scale-105 flex items-center gap-2 ${fs.sub} ${dark ? "bg-stone-900 text-amber-400 border border-stone-700" : "bg-white text-amber-600 border border-amber-200"}`}
        >
          <span>📍</span> {lang === "si" ? "සිතියම මත බලන්න" : "View on Google Maps"}
        </a>
      </div>
      {/* Contact Form */}
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${dark ? "bg-stone-950 text-stone-100" : "bg-amber-50 text-stone-800"}`}>
      ...
      {/* ── TOP NAVBAR ── */}
      <nav className={`sticky top-0 z-50 backdrop-blur-md border-b transition-colors duration-300 bg-stone-950/95 border-stone-800`}>
        <div className="max-w-6xl mx-auto px-4 py-2.5 flex items-center gap-3">
          {/* Brand */}
          <button onClick={() => handleNav("home")} className="flex items-center gap-2 flex-shrink-0">
            <span className="text-2xl">🍄</span>
            <span className={`font-extrabold tracking-tight font-serif hidden md:block text-amber-400 ${fs.heading}`}>{t.brand}</span>
          </button>

          {/* Search — flex-1 */}
          <div className="relative flex-1 max-w-xs hidden sm:block">
            <input type="text" placeholder={t.search} value={search} onChange={e => setSearch(e.target.value)}
              className={`w-full rounded-xl px-4 py-2 pr-9 outline-none ${fs.base} bg-stone-800 border border-stone-700 text-stone-100 placeholder-stone-500 focus:border-amber-500`} />
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <button onClick={() => setLang(l => l === "en" ? "si" : "en")}
              className={`px-3 py-1.5 rounded-xl font-bold transition-colors border ${fs.sub} bg-stone-800 border-stone-700 text-amber-400 hover:bg-stone-700`}>
              {lang === "en" ? "සිංහල" : "EN"}
            </button>

            {/* Font Size */}
            <div className={`hidden sm:flex rounded-xl overflow-hidden border border-stone-700`}>
              {["small", "medium", "large"].map((size, i) => (
                <button key={size} onClick={() => setFontSize(size)}
                  className={`px-2 py-1.5 font-bold transition-colors ${fontSize === size ? "bg-amber-600 text-white" : "bg-stone-800 text-stone-300 hover:bg-stone-700"} ${["text-xs", "text-sm", "text-base"][i]}`}>
                  A
                </button>
              ))}
            </div>
            ...
            <button onClick={() => setDark(d => !d)}
              className={`p-2 rounded-xl border transition-colors bg-stone-800 border-stone-700 text-amber-400 hover:bg-stone-700`}>
              {dark ? <Icon.Sun /> : <Icon.Moon />}
            </button>

            {/* Mobile Hamburger */}
            <button onClick={() => setMobileOpen(o => !o)}
              className={`md:hidden p-2 rounded-xl border transition-colors bg-stone-800 border-stone-700 text-stone-200`}>
        ...
        {/* ── SUB NAVBAR (desktop) ── */}
        <div className={`hidden md:block border-t border-stone-800`}>
          <div className="max-w-6xl mx-auto px-4 flex gap-1 py-1">
            {navPages.map(p => (
              <button key={p} onClick={() => handleNav(p)}
                className={`px-4 py-2 rounded-lg font-semibold transition-all capitalize ${fs.sub} ${page === p ? "bg-amber-600 text-white" : "text-stone-300 hover:bg-stone-800"}`}>
                {t.nav[p]}
              </button>
            ))}
          </div>
      ...
      {mobileOpen && (
        <div className={`fixed inset-0 z-40 flex`}>
          ...
          <div className={`relative z-10 w-64 h-full flex flex-col shadow-2xl bg-stone-900`}>
            <div className={`p-4 border-b border-stone-700 flex items-center gap-2`}>
              <span className="text-2xl">🍄</span>
              <span className={`font-extrabold font-serif text-amber-400`}>{t.brand}</span>
            </div>
            {/* Mobile Search */}
            <div className={`p-4 border-b border-stone-700`}>
              <div className="relative">
                <input type="text" placeholder={t.search}
                  className={`w-full rounded-xl px-4 py-2 pr-8 outline-none ${fs.base} bg-stone-800 border border-stone-700 text-stone-100 placeholder-stone-500`} />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm">🔍</span>
              </div>
            </div>
            {/* Font size mobile */}
            <div className={`p-4 border-b border-stone-700 flex items-center gap-2`}>
              <span className={`${fs.sub} text-stone-400`}>Font:</span>
              <div className={`flex rounded-lg overflow-hidden border border-stone-700`}>
                {["small", "medium", "large"].map((size, i) => (
                  <button key={size} onClick={() => setFontSize(size)}
                    className={`px-2.5 py-1 font-bold transition-colors ${fontSize === size ? "bg-amber-600 text-white" : "bg-stone-800 text-stone-300"} ${["text-xs", "text-sm", "text-base"][i]}`}>A</button>
                ))}
              </div>
            </div>
            {/* Nav links */}
            <nav className="flex-1 overflow-y-auto p-3 space-y-1">
              {navPages.map(p => (
                <button key={p} onClick={() => handleNav(p)}
                  className={`w-full text-left px-4 py-3 rounded-xl font-semibold transition-all capitalize ${fs.base} ${page === p ? "bg-amber-600 text-white" : "text-stone-200 hover:bg-stone-800"}`}>
                  {t.nav[p]}
                </button>
              ))}
            </nav>
      ...
      {/* ── FOOTER ── */}
      <footer className={`border-t mt-16 transition-colors duration-300 border-stone-800 bg-stone-900`}>
        <div className="max-w-5xl mx-auto px-4 py-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-16">
          {/* Brand col */}
          <div>
            ...
            <p className={`${fs.sub} text-stone-400 leading-relaxed`}>Celebrating the magic of fungi, one recipe at a time. Based in Colombo, Sri Lanka.</p>
          </div>
          {/* Quick Links */}
          <div>
            <h4 className={`font-bold mb-3 mt-2 ${fs.base} text-stone-200`}>Quick Links</h4>
            <ul className="space-y-1">
              {navPages.map(p => (
                <li key={p}><button onClick={() => handleNav(p)} className={`capitalize transition-colors ${fs.sub} text-stone-400 hover:text-amber-400`}>{t.nav[p]}</button></li>
              ))}
            </ul>
          </div>
          {/* Contact */}
          <div>
            <h4 className={`font-bold mb-3 mt-2 ${fs.base} text-stone-200`}>{t.contactTitle}</h4>
            <ul className="space-y-5">
              <li className={`flex items-center gap-3 ${fs.sub} text-stone-400`}><Icon.Clock />Mon–Sun, 9AM–5PM</li>
              <li>
                <a href="tel:0712518433" className={`flex items-center gap-3 hover:underline ${fs.sub} text-stone-400 hover:text-amber-400`}><Icon.Phone />0712518433</a>
              </li>
              <li>
                <a href="mailto:shenal@gmail.com" className={`flex items-center gap-3 hover:underline ${fs.sub} text-stone-400 hover:text-amber-400`}><Icon.Mail />shenal@gmail.com</a>
              </li>
            </ul>
          </div>
          {/* Social */}
          <div>
            <h4 className={`font-bold mb-3 mt-2 ${fs.base} text-stone-200`}>Follow Us</h4>
            <div className="flex gap-3 flex-wrap">
              {[<Icon.Facebook key="fb" />, <Icon.Youtube key="yt" />, <Icon.Tiktok key="tt" />, <Icon.WhatsApp key="wa" />].map((ic, i) => (
                <button key={i} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all hover:scale-110 bg-stone-800 hover:bg-stone-700 shadow`}>{ic}</button>
              ))}
            </div>
            <p className={`mt-4 ${fs.sub} text-stone-400`}>#MushroomKitchenSL</p>
          </div>
        </div>
        <div className={`border-t py-4 text-center ${fs.sub} border-stone-800 text-stone-600`}>
          © 2025 Mushroom Kitchen. All rights reserved.
        </div>
      </footer>

      {/* ── MOBILE BOTTOM NAV ── */}
      <div className={`fixed bottom-0 left-0 right-0 z-40 md:hidden border-t bg-stone-900 border-stone-800 pb-safe`}>
        <div className="flex">
          {navPages.slice(0, 5).map(p => {
            const icons = { home: "🏠", recipes: "🍳", shops: "🛒", contact: "📞", about: "👥", blog: "📝" };
            return (
              <button key={p} onClick={() => handleNav(p)}
                className={`flex-1 flex flex-col items-center py-2 transition-colors ${page === p ? "text-amber-400" : "text-stone-500"}`}>
          ...
          <button onClick={() => handleNav("blog")}
            className={`flex-1 flex flex-col items-center py-2 transition-colors ${page === "blog" ? "text-amber-400" : "text-stone-500"}`}>
